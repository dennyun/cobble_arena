...
public class ArenaSession {
    ...
    private long battleStartTimeMs = -1L;
    private long battleInitializationDeadlineTick = -1L;
    private boolean battleLaunchInProgress = false;
    private boolean mutualTimeoutEnded = false;
    private List<ArenaSession.TeamPokemonSnapshot> player1TeamSnapshot =
        List.of();
    private List<ArenaSession.TeamPokemonSnapshot> player2TeamSnapshot =
        List.of();
    private final List<UUID> player1SelectedLeadIds = new ArrayList<>();
    private final List<UUID> player2SelectedLeadIds = new ArrayList<>();
    private final Map<UUID, ArenaSession.DecisionTimerState> decisionTimers =
        new HashMap<>();
    ...
    public ArenaSession(
        ...
        this.decisionTimers.put(
            player1.getUuid(),
            new ArenaSession.DecisionTimerState()
        );
        this.decisionTimers.put(
            player2.getUuid(),
            new ArenaSession.DecisionTimerState()
    ...
    public ServerPlayerEntity getOpponent(ServerPlayerEntity player) {
        if (player.getUuid().equals(this.player1.getUuid())) {
            ...
            return player.getUuid().equals(this.player2.getUuid())
                ? this.player1
                : null;
        }
    }
    ...
    public long getBattleStartTimeMs() {
        return this.battleStartTimeMs;
    }
    ...
    public boolean hasBattleInitializationTimedOut(long currentTick) {
        return (
            this.battleInitializationDeadlineTick >= 0L &&
            currentTick >= this.battleInitializationDeadlineTick
        );
    }
    ...
    public boolean isMutualTimeoutEnded() {
        return this.mutualTimeoutEnded;
    }

    public void setMutualTimeoutEnded(boolean value) {
        this.mutualTimeoutEnded = value;
    }
    ...
    public boolean shouldStartBattle(long currentTick) {
        return (
            this.hasPendingBattleStart() &&
            (this.hasBothPlayersReadyLeads() ||
                currentTick >= this.pendingBattleStartTick)
        );
    }
    ...
    public void setSelectedLeads(
        ServerPlayerEntity player,
        List<UUID> pokemonIds
    ) {
    ...
    public int getRequiredLeadCount() {
        String battleTypeId =
            this.ladder != null ? this.ladder.getBattleTypeId() : "singles";
    ...
    public boolean hasMinimumLeadSelection(ServerPlayerEntity player) {
        ...
        return (
            selected != null && selected.size() >= this.getRequiredLeadCount()
        );
    }

    public boolean hasBothPlayersReadyLeads() {
        return (
            this.hasMinimumLeadSelection(this.player1) &&
            this.hasMinimumLeadSelection(this.player2)
        );
    }
    ...
    private List<UUID> selectedLeadList(ServerPlayerEntity player) {
        ...
        if (
            player.getUuid().equals(this.player1.getUuid())
        ) return this.player1SelectedLeadIds;
        if (
            player.getUuid().equals(this.player2.getUuid())
        ) return this.player2SelectedLeadIds;
        return null;
    }

    public void setTeamSnapshots(
        List<ArenaSession.TeamPokemonSnapshot> player1TeamSnapshot,
        List<ArenaSession.TeamPokemonSnapshot> player2TeamSnapshot
    ) {
    ...
    public List<ArenaSession.TeamPokemonSnapshot> getTeamSnapshot(
        ServerPlayerEntity player
    ) {
        if (player == null) {
            return List.of();
    ...
    public void teleportToArena() {
        ...
        ServerWorld arenaLevel = this.player1.getServer().getWorld(
            this.arena.getDimension()
        );
        if (arenaLevel == null) {
            throw new IllegalStateException(
                "Arena dimension not found: " +
                    this.arena.getDimension().getValue()
            );
        } else {
            ...
            double centerX =
                (player1BattlePos.getX() + player2BattlePos.getX()) / 2.0 + 0.5;
            double centerY = player1BattlePos.getY();
            double centerZ =
                (player1BattlePos.getZ() + player2BattlePos.getZ()) / 2.0 + 0.5;
            this.teleportPlayerToView(
                this.player1,
                this.player1ViewPos,
                this.arena.getDimension(),
                centerX,
            ...
            this.healParty(this.player1);
            this.healParty(this.player2);
        }
    }

    private void healParty(ServerPlayerEntity player) {
        if (player == null) return;
        try {
            com.cobblemon.mod.common.Cobblemon.INSTANCE.getStorage().getParty(player).heal();
        } catch (Exception e) {
            cobblemon.arena.CobblemonArena.LOGGER.error("Erro ao curar pokemons de " + player.getName().getString(), e);
        }
    }
    ...
    public void restorePlayers() {
        restorePlayerSafe(
            this.player1,
            this.player1OriginalGameMode,
            this.player1OriginalFovEffects,
            this.player1State
        );
        restorePlayerSafe(
            this.player2,
            this.player2OriginalGameMode,
            this.player2OriginalFovEffects,
            this.player2State
        );
    }

    private void restorePlayerSafe(
        ServerPlayerEntity player,
        GameMode originalGameMode,
        boolean originalFovEffects,
        PlayerState state
    ) {
        if (player == null) return;
        ...
        try {
            player.getAbilities().allowFlying =
                originalGameMode == GameMode.CREATIVE ||
                originalGameMode == GameMode.SPECTATOR;
            player.sendAbilitiesUpdate();
        } catch (Exception ignored) {}
        try {
            this.restoreFovEffects(player, originalFovEffects);
        } catch (Exception ignored) {}
        // Registra restore offline ANTES de tentar teleportar
        // Se o jogador estiver offline, o dado persiste no arquivo para o próximo login
        try {
            cobblemon.arena.arena.OfflineRestoreManager.getInstance().register(
                player.getUuid(),
                state.getOriginalPosition(),
                state.getOriginalDimension(),
                state.getOriginalYRot(),
                state.getOriginalXRot()
            );
        } catch (Exception ignored) {}
        ...
        try {
            ...
            player.setYaw(state.getOriginalYRot());
            player.setPitch(state.getOriginalXRot());
            // Teleporte online bem-sucedido — remove do registro offline
            cobblemon.arena.arena.OfflineRestoreManager.getInstance().clear(
                player.getUuid()
            );
        } catch (Exception ignored) {
            // Jogador offline — o restore será aplicado no próximo login via JOIN handler
        }
    }

    private void teleportPlayerToView(
        ServerPlayerEntity player,
        BlockPos pos,
        ...
        if (targetLevel == null) {
            throw new IllegalStateException(
                "Arena dimension not found: " + dimension.getValue()
            );
        } else {
    ...
    private void teleportPlayer(
        ...
        if (targetLevel == null) {
            throw new IllegalStateException(
                "Target dimension not found: " + dimension.getValue()
            );
        } else {
    ...
    private void applyMovementLock(ServerPlayerEntity player) {
        EntityAttributeInstance movementSpeed = player.getAttributeInstance(
        ...
        EntityAttributeInstance jumpStrength = player.getAttributeInstance(
            EntityAttributes.GENERIC_JUMP_STRENGTH
        );
        if (jumpStrength != null) {
            Identifier jumpModifierId = Identifier.of(
                "cobblemon_arena",
                "jump_lock"
            );
    ...
    private ArenaSession.DecisionTimerState timerState(
        ServerPlayerEntity player
    ) {
        return player != null
            ? this.decisionTimers.get(player.getUuid())
            : null;
    }

    private static List<ArenaSession.TeamPokemonSnapshot> immutableTeamSnapshot(
        List<ArenaSession.TeamPokemonSnapshot> entries
    ) {
        if (entries != null && !entries.isEmpty()) {
            List<ArenaSession.TeamPokemonSnapshot> copy = new ArrayList<>(
                entries.size()
            );
    ...
    public static final class TeamPokemonSnapshot {
        ...
        public TeamPokemonSnapshot(
            ...
            this.typeNames =
                typeNames == null ? List.of() : List.copyOf(typeNames);
            this.moveNames =
                moveNames == null ? List.of() : List.copyOf(moveNames);
